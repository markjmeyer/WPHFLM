...
%% scale to control STNR %%
b1ca    = (1/(sqrt(2*pi*b1var2)))*exp(-1/(2*b1var2)*(tc./T-0*vc./T-0.1).^2);
b1cb    = (1/(sqrt(2*pi*b1var1)))*exp(-1/(2*b1var1)*(tc./T-0*vc./T-0.2).^2);

%%
b2c      = (1/max(max(b1ca)))*b1ca - (1/max(max(b1cb)))*b1cb;
...
%% subset historical coefs %%
bhtc     = (1/abs(min(bhrc)))*bhrc(bFlagc == 1);
...
%% scale to control STNR %%
b1r     = stnrs2*(1/(sqrt(2*pi*b1var2)))*exp(-1/(2*b1var2)*(tr./T-0*vr./T-0.1).^2)-stnrs1*(1/(sqrt(2*pi*b1var1)))*exp(-1/(2*b1var1)*(tr./T-0*vr./T-0.2).^2);
