...
if strcmp(type1,'bspline')       & strcmp(type2,'bspline')      & ...
  ...
  %  set up first iteration
    ...
    basismat1 = getbasismatrix(basis1, rng, Lfdobj1);
    basismat2 = getbasismatrix(basis2, rng, Lfdobj2);
    if ~isnumeric(wtfd)
        wtd = eval_fd(wtfd, rng);
        basismat2 = (wtd * ones(1,nbasis2)) .* basismat2;
    end
    s(1,:,:)  = width.*(basismat1' * basismat2)./2;
    tnm = 0.5;
    ...
    for iter = 2:JMAX
        ...
        basismat1 = getbasismatrix(basis1, x, Lfdobj1);
        basismat2 = getbasismatrix(basis2, x, Lfdobj2);
        if ~isnumeric(wtfd)
            wtd = eval_fd(wtfd, x);
            basismat2 = (wtd * ones(1,nbasis2)) .* basismat2;
        end
        chs = reshape(width.*(basismat1' * basismat2)./tnm,[1,nbasis1,nbasis2]);
