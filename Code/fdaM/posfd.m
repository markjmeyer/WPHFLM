...
%  ensure that both x and f are column vectors

if size(x,1) > 1 & size(x,2) > 1
    error('Argument X is not a vector.');
end
if size(f,1) > 1 & size(f,2) > 1
    error('Argument F is not a vector.');
end
x = x(:);
f = f(:);
...
[logl, Dlogl] = loglfnpos(x, f, basis, cvec0);
...
hmat = loglhesspos(x, f, basis, cvec0);
if lambda > 0
    hmat = hmat + 2.*Kmat;
end
...
for iter = 1:iterlim
     ...
     Fstr = Foldstr;
      ...
      for stepiter = 1:STEPMAX
        ...
        [logl, Dlogl] = loglfnpos(x, f, basis, cvecnew);
        Fstr.f  = -logl;
        gvecnew = -Dlogl;
        if lambda > 0
     ...
     %  compute the Hessian
     hmat = loglhesspos(x, f, basis, cvec);
     if lambda > 0
        hmat = hmat + 2.*Kmat;
     end
...
function [logl, Dlogl] = loglfnpos(x, f, basis, cvec)
    N = length(x);
    nbasis  = getnbasis(basis);
...
function D2logl = loglhesspos(x, f, basis, cvec)
    N = length(x);
    nbasis  = getnbasis(basis);
    phimat  = getbasismatrix(x, basis);
    Wvec    = phimat*cvec;
